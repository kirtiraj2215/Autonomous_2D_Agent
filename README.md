...
- The exploration rate (ε) decays over time, so the agent explores more at the beginning and gradually exploits its learned knowledge more as training progresses.
...
During training, the agent initially explored the environment too much, which led to slow learning. Balancing the exploration rate (ε) was critical to achieving optimal performance. We experimented with different decay rates for (ε) to find an appropriate balance between exploration and exploitation.
